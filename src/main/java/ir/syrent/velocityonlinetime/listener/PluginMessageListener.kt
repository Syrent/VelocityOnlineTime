...
class PluginMessageListener(
    ...
    private fun onPluginMessage(event: PluginMessageEvent) {
        ...
        Database.getPlayerOnlineTime(player).whenComplete { time, _ ->
            ...
            VRUoMPlugin.getServer().allPlayers.iterator().next().currentServer.get().sendPluginMessage(VelocityOnlineTime.VELOCITYONLINETIME_CHANNEL, byteArrayDataOutput.toByteArray())
        }
    }
}