package ir.syrent.velocityonlinetime.velocity.storage.sqlite

import com.velocitypowered.api.scheduler.ScheduledTask
import ir.syrent.velocityonlinetime.velocity.VelocityOnlineTime
import ir.syrent.velocityonlinetime.velocity.storage.sqlite.exception.SQLiteException
...
open class SQLiteDatabase(dbFile: File?) : SQLiteExecutor(dbFile, Logger.getLogger(VRuom.getPlugin().name)) {
    ...
    private fun startQueue(): ScheduledTask {
        ...
        return VRuom.getServer().scheduler.buildTask(VelocityOnlineTime.instance, runnable).schedule()
    }
